...
impl GrappaKernel {
    ...
    pub fn calibrate(
                ...
                for ch in 0..nc {
                    for kky in 0..kernel_ky {
                        ...
                        for kkx in 0..kernel_kx {
                            let src_x = kx0 + kkx;
                            let col = ch * (kernel_ky * kernel_kx) + kky * kernel_kx + kkx;
        ...
        let lam = {
            ...
            let mean_diag =
                (0..n_src).map(|i| ata_reg[[i, i]].re).sum::<f32>() / (n_src as f32).max(1.0);
...
mod tests {
    ...
    fn grappa_reconstructs_r2_from_fully_sampled_phantom() {
        ...
        let kernel = GrappaKernel::calibrate(acs.view(), 2, 4, 5, 1e-3).expect("calibrate ok");
